...
void GameManager::start()
{
	...
	while (currentState != exitState)
	{
		...
		bool shouldRefresh = didTransitionOccurLastFrame;
		for (UserInterface* ui : userInterfaces[currentState])
		{
			if (ui->isDirty())
				shouldRefresh = true;
		...
		// Since refreshing the screen can be expensive/laggy, only want
		// to refresh when there's been an update to the UI
		if (shouldRefresh)
		{
			clear_screen();
